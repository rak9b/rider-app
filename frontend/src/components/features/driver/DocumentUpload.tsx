...
export const DocumentUpload = () => {
  const [documents, setDocuments] = useState<Document[]>([
    { id: '1', name: 'Driver\'s License', status: 'verified', expiry: '2027-12-31' },
    { id: '2', name: 'Vehicle Insurance', status: 'pending' },
    { id: '3', name: 'Vehicle Registration', status: 'missing' },
    { id: '4', name: 'Background Check', status: 'verified', expiry: '2024-06-15' }, // Expired!
  ]);
  ...
  const isDocExpired = (doc: Document) => {
  ...
  const getEffectiveStatus = (doc: Document) => {
    if (doc.status === 'verified' && isDocExpired(doc)) {
      return 'expired';
    }
    return doc.status;
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 px-4 py-2 rounded-lg text-sm font-medium">
          {documents.filter(d => getEffectiveStatus(d) === 'verified').length} / {documents.length} Completed
        </div>
      </div>
      ...
      <div className="grid gap-4">
        {documents.map((doc) => {
          ...
          return (
            <Card key={doc.id} className={`hover:shadow-md transition-shadow ${isExpired ? 'border-red-300 dark:border-red-900/50 bg-red-50/20' : ''}`}>
              <CardContent className="p-4 sm:p-6 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${
                    status === 'verified' ? 'bg-green-100 dark:bg-green-900/20' : 
                    status === 'expired' ? 'bg-red-100 dark:bg-red-900/20' : 
                    status === 'missing' ? 'bg-gray-100 dark:bg-slate-800' : 'bg-yellow-100 dark:bg-yellow-900/20'
                  }`}>
                    <FileText className={`w-6 h-6 ${
                      status === 'verified' ? 'text-green-600' : 
                      status === 'expired' ? 'text-red-600' : 
                      status === 'missing' ? 'text-gray-400' : 'text-yellow-600'
                    }`} />
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {doc.expiry && (
                        <span className={`text-xs ${isExpired ? 'text-red-600 font-bold' : 'text-gray-500'}`}>
                          Expires: {doc.expiry} {isExpired ? '(EXPIRED)' : ''}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-4">
                  {status !== 'verified' && (
                    <Button 
                      variant={status === 'missing' || status === 'expired' ? 'primary' : 'outline'} 
                      size="sm"
                      onClick={() => handleUpload(doc.id)}
                    >
                      <Upload className="w-4 h-4 mr-2" />
                      {status === 'missing' ? 'Upload' : isExpired ? 'Renew Document' : 'Re-upload'}
                    </Button>
                  )}
                  {status === 'verified' && (
